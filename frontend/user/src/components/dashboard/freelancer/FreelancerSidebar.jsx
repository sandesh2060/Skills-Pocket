    ...
    try {
      ...
      if (
        ...
        if (retryCountRef.current < maxRetries) {
          ...
          console.log(
            `⏳ Retrying in ${delay}ms (attempt ${retryCountRef.current + 1}/${maxRetries})`,
          );
  ...
  const NavLink = ({ item, onClick }) => (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <span
          className={`text-sm ${isActive(item.path) ? "font-semibold" : "font-medium"} truncate block`}
        >
          {item.label}
        </span>
        {item.subLabel && (
      ...
      {item.badge !== null && item.badge !== undefined && item.badge > 0 && (
        <span
          className={`${item.badgeColor} text-white text-[10px] px-1.5 py-0.5 rounded-full flex-shrink-0 font-bold min-w-[20px] text-center`}
        >
        ...
        <div className="p-6 border-b border-[#e7edf3] dark:border-slate-800">
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col min-w-0">
              <h1 className="text-[#0d141b] dark:text-white text-lg font-bold leading-tight truncate">
                SkillsPocket
              </h1>
              <p className="text-[#4c739a] dark:text-slate-400 text-xs font-normal truncate">
                Freelancer Portal
              </p>
            </div>
          </div>
